...
export class IazoService {
  ...
  async createIazo(iazoDto: Iazo, file: Express.Multer.File) {
    ...
    if (uniqueIazo.length > 0) throw new BadRequestException('Iazo already exists');
    let uploadFile = {
      url: '',
    };

    const fileSize = file.size / 1024 / 1024; // in MiB
    if (fileSize > this.maxUploadSizeMb) throw new BadRequestException('Image larger than 2MB');
    try {
  ...
  async approveIazo(_id, approveIazoDto) {
    const subject = approveIazoDto.status === 'Rejected' ? 'Rejected IAZO' : 'Approved IAZO';
    const data = {
  ...
  async validateAddressIazo(address) {
    ...
    const isRegistered = await iazoContract.methods.IAZOIsRegistered(address).call();
