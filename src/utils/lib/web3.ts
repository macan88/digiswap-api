...
function getRandomWeb3(chainId: number) {
  ...
  if (!web3[provider]) {
    web3[provider] = new Web3(new Web3.providers.HttpProvider(provider, { timeout: 30000 }));
    console.log(`Initialized web3 provider: ${provider}`);
  }
  return web3[provider];
}

export const getWeb3 = (chainId = 56): Web3 => {
...
export const getContract = (abi: any, address: string, chainId: number = 56) => {
...
export const isTransactionMined = async (transactionHash) => {
  try {
    const txReceipt = await getWeb3().eth.getTransactionReceipt(transactionHash);
...
export const getCurrentBlock = async (chainId: number = 56) => {
  return await getRandomWeb3(chainId).eth.getBlockNumber();
};
