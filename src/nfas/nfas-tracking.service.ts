...
export class NfasTrackingService {
  ...
  wbnbTransferedAbi = ['event Transfer(address indexed src, address indexed dst, uint wad);'];
  ...
  async getNfaTransactionHistoryDescendingBlock(index: number): Promise<NfaTrackingDocument[]> {
  ...
  async parseZeroValue(event) {
    let value = 0;
    const transactionReceipt = await this.provider.getTransactionReceipt(event.transactionHash);
